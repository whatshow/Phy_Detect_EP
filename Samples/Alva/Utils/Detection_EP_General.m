...
% 3.The variance of each input can be configured
% -------------------------------------------------------------------------
% OUTPUTS
%
...
function [syms] = Detection_EP_General(sympool, sympool_var, SNR, H, y, y_var, ep_iter_times, beta)
    %syms = zeros(, 1, ep_iter_times);

    % -------------------------------------------------------------------------
    % DEBUG
    % -------------------------------------------------------------------------
    % debug, 1 is on, 0 is off
    debug = 0;
    % if 'DbgPrint' don't exist, we'll define that function which does nothing
    if ~exist("DbgPrint")
        DbgPrint = @(debug, varagin) 1;
    end

    % -------------------------------------------------------------------------
    % DEV
    % -------------------------------------------------------------------------
    % Set inputs to default values if those inputs are NOT GIVEN!
    % Error if inputs are not enough
    if nargin < 7
        error("Detection_EP: Inputs are not enough!");
    end
    % If no input of beta, beta will be 0.2
    if nargin < 8
        beta = 0.2;
    end
    pxy_pred_var_min = 5*10^-7;
    ...
    for i = 1:1:ep_iter_times
        ...
        pxy_covar_TMP = inv((H')*H + y_var*diag(px_Lambda));
        pxy_covar = y_var*pxy_covar_TMP;
        pxy_mean = pxy_covar_TMP*(H'*y + y_var*px_gamma);
